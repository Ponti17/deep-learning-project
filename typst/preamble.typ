#let make-venue = move(dy: -1.9cm, {
  box(rect(fill: luma(140), inset: 10pt, height: 2cm)[
    #set text(font: "TeX Gyre Pagella", fill: white, weight: 700, size: 20pt)
    #align(bottom)[Aarhus University]
  ])
})
...
#let make-title(
  ...
  [
    #heading(outlined: false, bookmarked: false)[Introduction]
...
#let template(
    ...
    place(
      make-title(title, authors, abstract, keywords, date), 
      top, 
      scope: "parent",
      float: true
    )
