...
def main():
    ...
    # Load the Neptune API key from the very _secret_ file
    ...
    for epoch in range(config['TRAIN']['EPOCHS']):
        # Log epoch to Neptune
        run["training/epoch"].log(epoch + 1)

        accumulated_output = {}
        for step_idx, data in enumerate(train_dataloader):
            # Train loop
            # A result dict is returned but not used for anything
            # Contains the overall loss
        ...
        for step_idx, data in enumerate(val_dataloader):
            ...
            # Continously update the results from validation
            update_accumulated_output(accumulated_output, valid_result_dict)

        lr_scheduler.step()
        # Process the acuumulated validation output and determine dice coefficients
        ...
        # Print statement so we know where we are when something fails on LUMI.
        print(
